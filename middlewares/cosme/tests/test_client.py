...
from middlewares.cosme.config import (
    DEFAULT_YEAR,
    archive_root_url,
)
...
async def test_input_translation_default() -> None:
    ...
    assert len(seeds) == 1
    seed = seeds[0]
    assert seed["url"] == archive_root_url(DEFAULT_YEAR)
    assert seed["award_year"] == DEFAULT_YEAR
    assert seed["crawl_limit"] == 24
...
async def test_input_translation_with_year_and_category() -> None:
    ...
    assert seeds == [
        {
            "url": "https://www.cosme.net/bestcosme/archive/2025/",
            "award_year": 2025,
            "crawl_limit": 5,
            "category": "serum",
        }
