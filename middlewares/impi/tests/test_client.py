...
top-level. The ``middlewares/impi/conftest.py`` mirrors the production
...
def _ensure_impi_scraper_mx_path() -> None:
    ...
    Defined in ``middlewares.impi.client`` — kept there so the production
...
async def test_module_level_trigger_and_get(monkeypatch: pytest.MonkeyPatch) -> None:
    """``from middlewares.impi import trigger, get_result`` works end-to-end.
    ...
    from middlewares.impi import client as client_mod
...
async def test_harness_discovery_contract() -> None:
    ...
    """
    import middlewares.impi as pkg
