...
Atypical vs the rest of ``middlewares/``:
...
Public API (contract consumed by the agents harness):
    ...
    >>> from middlewares.impi import trigger, get_result
    >>> job = await trigger({"owner": "Genomma", "expires_within_days": 90})
    >>> res = await get_result(job["job_id"])

The error catalog is the base one from ``middlewares.core.errors``; portal
...
from middlewares.impi.config import DEFAULT_ETA_SECONDS, SOURCE_NAME
...
def _ensure_impi_scraper_mx_path() -> None:
    ...
    present on ``sys.path``.
    """
    package_root = str(Path(__file__).resolve().parents[2] / "impi_scraper_mx")
...
# inside the methods that need it so that importing ``middlewares.impi`` at
...
def _default_client_factory() -> Any:
    ...
    Imported lazily so that ``from middlewares.impi import trigger`` works even
