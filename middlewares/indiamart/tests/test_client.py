...
from middlewares.indiamart import (
    TOOL_SCHEMA,
    IndiamartClient,
    IndiamartInputs,
    ProductRow,
...
from middlewares.indiamart.config import (
...
async def test_input_translation_default() -> None:
    ...
    mcat_slugs = [s["category_slug"] for s in mcat_seeds]
    for slug in DEFAULT_MCAT_SLUGS:
        assert slug in mcat_slugs
    # Every default industry hub present.
    hub_slugs = [s["category_slug"] for s in hub_seeds]
    for hub in DEFAULT_INDUSTRY_HUBS:
        assert hub in hub_slugs
...
async def test_input_translation_explicit_mcats() -> None:
    ...
    assert seeds == [
        {"category_slug": "caustic-soda", "kind": "mcat"},
    ]
...
async def test_input_translation_explicit_hubs() -> None:
    ...
    assert {"category_slug": "caustic-soda", "kind": "mcat"} in seeds
    assert {"category_slug": "chem", "kind": "hub"} in seeds
    assert len(seeds) == 2
...
async def test_input_translation_hubs_disabled() -> None:
    ...
    assert len(seeds) == 1
    assert seeds[0] == {"category_slug": "caustic-soda", "kind": "mcat"}
