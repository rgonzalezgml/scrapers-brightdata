...
from gli_scrapers.olive_young import (
    TOOL_SCHEMA,
    BrandRow,
    OliveYoungClient,
    OliveYoungInputs,
...
async def test_input_translation_default() -> None:
    ...
    assert len(seeds) == 1
    seed = seeds[0]
    assert "oliveyoung.com" in seed["url"]
    assert "target=" in seed["url"]
    assert seed["region"] == "kr"
    assert "regions" not in seed
    assert "max_brand_visits" not in seed
...
async def test_input_translation_with_categories_and_regions() -> None:
    ...
    assert len(seeds) == 1
    assert seeds[0]["region"] == "kr"
    assert "oliveyoung.com" in seeds[0]["url"]
