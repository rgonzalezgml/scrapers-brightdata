"""Pytest config for the cosme middleware tests.
...
- Skips live BrightData tests when env vars are unset. Dual-mode: a live
  test is runnable if EITHER ``BRIGHTDATA_DATASET_ID_COSME`` (v3) OR
  ``BRIGHTDATA_COLLECTOR_ID_COSME`` (DCA) is populated alongside
...
# ``scrapers/cosme/sc_code/parser_code_v2.js`` emits for product pages, plus
# a couple of ranking and brand rows for heterogeneity coverage. See the
# README in the fixtures dir for the construction notes.
SNAPSHOT_FIXTURE = FIXTURES_DIR / "cosme_snapshot_s_demo01.json"
...
def pytest_collection_modifyitems(config, items):
    ...
    has_dataset = bool(os.getenv("BRIGHTDATA_DATASET_ID_COSME"))
    has_collector = bool(os.getenv("BRIGHTDATA_COLLECTOR_ID_COSME"))
    if has_key and (has_dataset or has_collector):
        return
    skip_reason = (
        "BRIGHTDATA_API_KEY and (BRIGHTDATA_DATASET_ID_COSME or "
        "BRIGHTDATA_COLLECTOR_ID_COSME) not set — skipping live "
        "BrightData tests."
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        ...
        "BRIGHTDATA_DATASET_ID_COSME or "
        "BRIGHTDATA_COLLECTOR_ID_COSME are set).",
    )
