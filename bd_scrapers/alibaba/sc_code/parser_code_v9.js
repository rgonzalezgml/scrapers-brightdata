// parser_code_v9 — Alibaba detail page (Code worker).
//
// Cambios sobre parser_code_v8.js (delta v9):
//
//   (fix-array) JSON-LD array top-level causa parse error en v8.
//       priceFromJsonLd() usaba `data['@graph'] ? data['@graph'] : [data]`,
//       que falla cuando `data` es un JSON array (sin clave @graph).
//       Causa del TypeError "Cannot read properties of null (reading 'price_raw')"
//       en run 09: 373 filas (26.9%) crashearon y no tienen campos de precio.
//       Fix: Array.isArray() en scanJsonLd() y FB4.
//
//   (price_raw-dom) price_raw siempre texto original del DOM.
//       v8 construía strings sintéticos como "USD20-USD25" desde JSON-LD,
//       mezclando texto visible con datos estructurados.
//       Fix: reemplazar priceFromJsonLd() por scanJsonLd() que retorna valores
//       ya parseados {min, max, currency}. Cascada DOM P1–P6 corre primero y
//       popula price_raw con texto original. Si DOM no encuentra precio
//       (hydration timeout), scanJsonLd() asigna priceParsed directamente con
//       price_raw = "[jsonld: CUR{low}]" como marcador distinguible.
//
//   (ladder) JSON-LD single-price bloqueaba P1 (.price-item).
//       En v8, P0 disparaba primero con el tier mínimo del JSON-LD ("20"),
//       impidiendo que P1 leyera el rango completo del DOM (20–25).
//       Fix: JSON-LD solo corre como P7 (último fallback).
//
