// parser_code_v2 — Alibaba detail page (Code worker).
//
// Cambios vs parser_code_v1.js:
//   v1 emitía flags `scraper_flags: skipped_<reason>` (missing_name,
//   missing_price, machinery, non_ascii_title) dentro del return, como
//   workaround al runtime de Scraper Studio que rechaza `parse()` = null.
//   v2 asume `interaction_code_v2.js` — que hace los skips via `dead_page()`
//   ANTES de llamar a parse() (URL shape, bot-check) o DESPUÉS validando
//   invariantes (product_url + cleaned_product_name). Por eso el parser
//   ya no inventa flags de skip: la responsabilidad vive en el interaction.
//
// Dependencia de archivos:
//   parser_code_v2.js  ⇔  interaction_code_v2.js
//   NO usar con interaction_code_v1 (pierde los guards).
//   NO usar parser_code_v1 con interaction_code_v2 (duplica lógica).
//
// Output Schema Fase 1 — SIN CAMBIOS: mismas 10 keys vendor-literales que
// v1 (cleaned_product_name, product_url, supplier_name, price_raw,
// price_min_usd, price_max_usd, price_unit, minimum_order_quantity,
// cas_number, purity). La migración a las 16 keys spec §2 se hace en una
// release coordinada separada (Fase 2).
//
// Fixes que v2 hereda de v1:
...
// declarado en el dashboard. Las 10 keys Fase 1 deben existir en ese schema;
// si falta alguna el runtime lanza parse validation error (R9).
...
// `flags[]` + `addFlag()` se conservan porque `cleanName()` emite
// `name_clean_fallback` y `detectUnit()` puede emitir `price_unit_unknown` /
// `price_fx_needed`. Estos flags son internos (debugging) y NO salen en el
// return; v2 dropeó el bloque de skip rules que antes los exponía como
// `scraper_flags` en el output. Ver interaction_code_v2.js para skip flow.
const flags = [];
...
// ---------------------------------------------------------------------------
// Invariantes §4 (traza interna, NO skip).
//
// Las skip rules de v1 (missing_name, missing_price, machinery,
// non_ascii_title) se movieron a `interaction_code_v2.js` vía `dead_page()`.
// Los dos addFlag de abajo son puramente diagnósticos (si el HTML carga pero
// no se encontró h1 / canonical), quedan en memoria y NO salen en el return.
// No emiten `scraper_flags` al Output Schema — Fase 1 es 10 keys estrictas.
// ---------------------------------------------------------------------------
