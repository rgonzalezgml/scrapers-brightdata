// parser_code_v4 — Alibaba detail page (Code worker).
//
// Fixes vs parser_code_v3.js (arregla E1):
//
//   (E1-price) Tercer fallback de precio para el template "tiered-price"
//       (~2026-05-03). Cuando `.price-item` está ausente Y ninguno de los
//       fallbacks de v3 matchea (ni `.price-range` etc., ni el regex
//       "minimum order quantity"), se escanea el bloque de descripción del
//       supplier buscando TODOS los montos con símbolo de moneda mediante:
//           /(NZ\$|CA\$|A\$|US\$|R\$|B\/\.|RM|Rp|[€£¥₹₩฿$])\s*[\d.,]+/g
//       Se construye un rango "min-max" (precio más bajo primero). Si solo
//       hay un precio, se usa directamente. El MOQ se extrae del primer
//       threshold de cantidad+unidad que aparece antes del primer precio en
//       el mismo bloque de texto.
//
//   (E1-supplier) Fallback adicional para supplier_name en el template
//       tiered-price. Tras el fallback existente `a[href*=".en.alibaba.com"]`,
//       se intenta `a[href*="alibaba.com"][href*="/index"]`. La correlación
//       perfecta supplier-null ↔ price-null en los datos indica que en el
//       nuevo template el link del supplier no tiene el subdominio
//       *.en.alibaba.com.
//
// Todos los demás comportamientos son idénticos a v3.
...
if (priceItems.length > 0) {
    ...
    if (!price_raw) {
        const CURRENCY_RE = /(NZ\$|CA\$|A\$|US\$|R\$|B\/\.|RM|Rp|[€£¥₹₩฿$])\s*[\d.,]+/g;
