// parser_code_v6 — Alibaba detail page (Code worker).
//
// Fixes vs parser_code_v5.js:
//
...
//       text_sane() devuelve "$698-1770" (incorrecto). Fix: cuando .price-item
//       está ausente, intentar [data-testid="range-price"] ANTES de los
//       fallbacks de texto. Se recolectan los spans, se reconstruye el precio
//       insertando el punto decimal entre el integer y el decimal de cada parte.
//
...
//
// Todos los demás comportamientos son idénticos a v4.
...
// --- Precio (cascada §4 + template customizable + template tiered-price). -
// Fixed-price: los .price-item tienen tiers (1-24 kg → $X, 25-99 → $Y).
// Customizable-MOQ: el precio está en el texto del bloque "Product descriptions
// from the supplier" con shape:
//   Minimum order quantity: 1 unit
//   $12,000
// Tiered-price (E1, nuevo ~2026-05-03): muestra rangos de cantidad → precio
// en texto libre, sin "Minimum order quantity":
//   1,000 - 4,999 kilograms
//   US$1.70
//   >= 5,000 kilograms
//   US$1.63
// Range-price split-decimal (C6, nuevo ~2026-05-06): header con
//   [data-testid="range-price"] cuyos spans NO tienen punto decimal.
let price_raw = null;
let moq_raw = null;

const priceItems = $(".price-item").toArray();
if (priceItems.length > 0) {
    // C7: helper que soporta el selector legacy Y el nuevo split-decimal.
    ...
    const first = priceFromItem(priceItems[0]);
    const last = priceFromItem(priceItems[priceItems.length - 1]);
    if (first && last) {
    ...
    // Fix C6 — Template range-price split-decimal [spec §4c].
    // Intentar [data-testid="range-price"] ANTES de los fallbacks de texto.
    // Los spans no tienen punto decimal; se reconstruye insertándolo.
    if (!price_raw) {
        ...
        if (rpEl.length) {
            ...
            if (spans.length >= 3) {
                ...
                price_raw = spans.length >= 5
                    ? `${sym}${int1}.${dec1}-${sym}${int2}.${dec2}`
                    : `${sym}${int1}.${dec1}`;
            }
            if (!moq_raw) {
                const moqText = rpEl.find('[class*="text-sm"]').first().text_sane();
                if (moqText) moq_raw = moqText;
            }
        }
    }
    ...
    // Fallback customizable-MOQ: buscar en el bloque de descripción del supplier.
    ...
    // Fallback tiered-price (E1): sin .price-item, sin "minimum order quantity".
    // Escanear el bloque descripción buscando todos los montos con símbolo de
    // moneda. Si se encuentran ≥1 precios, construir rango min-max.
    // MOQ = primer threshold de cantidad+unidad que precede al primer precio.
    //
    // v5: CURRENCY_RE ampliada (C2+C3): añade SEK/DKK/RSD/円 como prefijo,
    // añade segunda alternativa sufijo (TL/TRY/SEK/DKK/RSD/円/€), flag gi.
    if (!price_raw) {
        const CURRENCY_RE = /(NZ\$|CA\$|A\$|US\$|R\$|B\/\.|RM|Rp|SEK|DKK|RSD|[€£¥₹₩฿$円])\s*[\d.,]+|[\d.,]+\s*(TL|TRY|SEK|DKK|RSD|円|€)/gi;
        const MOQ_RE = /([\d,]+)\s*(?:-\s*[\d,]+)?\s*(kilogram|kilograms|kg|ton|tons|tonne|tonnes|piece|pieces|unit|units|liter|liters|litre|litres)/i;
        const tieredCandidates = $(
            'section, div[class*="description"], div[class*="detail"], [class*="supplier-desc"]'
        ).toArray();
        for (const el of tieredCandidates) {
            const txt = $(el).text_sane();
            if (!txt) continue;
            ...
            while ((pm = CURRENCY_RE.exec(txt)) !== null) {
                priceMatches.push(pm[0].trim());
            }
            if (priceMatches.length === 0) continue;
            // Necesitamos al menos un precio para considerar este bloque.
            ...
            // Obtener el string del primer match que tiene el precio mínimo
            // y el primero que tiene el precio máximo para preservar el símbolo.
            ...
            if (minVal === maxVal) {
                ...
                price_raw = `${minMatch}-${maxMatch}`;
            }
            // MOQ: buscar el primer threshold de cantidad con unidad en el bloque,
            // antes del primer precio.
            if (!moq_raw) {
                const firstPriceIdx = txt.search(CURRENCY_RE);
                const textBeforePrice = firstPriceIdx > 0 ? txt.slice(0, firstPriceIdx) : txt;
                const moqM = textBeforePrice.match(MOQ_RE);
                if (moqM) moq_raw = moqM[0].trim();
            }
            addFlag("tiered_price_fallback");
            break;
        }

        // F1+F4: body fallback con ancla de sección + strip de RTL chars (UI hebrea).
        if (!price_raw) {
            ...
            if (bMatches.length > 0) {
                ...
                if (bValues.length > 0) {
                    ...
                    price_raw = bMin === bMax ? bMinMatch : `${bMinMatch}-${bMaxMatch}`;
                    if (!moq_raw) {
                        ...
                        if (moqMatch) moq_raw = moqMatch[0].trim();
                    }
