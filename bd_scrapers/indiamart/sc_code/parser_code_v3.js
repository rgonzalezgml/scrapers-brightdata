// parser_code_v3.js — IndiaMART Stage 2 product detail (Code worker).
// JSON-LD-primero. Superset completo §4+§5 de la spec (2026-04-23).
//
// Contexto del cambio (2026-04-23):
// parser_code_v2 era vendor v1 con un único hotfix del selector de
// supplier_url/supplier_id. Emitía 20 fields con selectores obsoletos y
// derivaba `type` de una fila "Bulb Type" (artefacto lámparas, no
// quimicos). El fixture canónico §11 (22408594448) expuso 9 bugs de DOM
// y 1 campo derivado mal (`type`). v3 refactoriza a extracción
// JSON-LD-primero y cierra los 10 bugs según tabla §3 del handoff
// `/workspace/docs/fase3/indiamart-parser-v3-handoff.md`.
//
// Diff semántico vs v2:
// - 3 bloques JSON-LD iterados con `.toArray().map(...)` (R12). v2
//   tomaba `$('script[...]').html()` sin each → solo el BreadcrumbList,
//   nunca el Product.
// - `type` derivado del BreadcrumbList position=2 `.@id` (ind_chem →
//   chemical, ind_packaging → packaging, resto other). v2 leía fila
//   "Bulb Type" inexistente.
// - `category_path`, `category_mic`, `industry_slug` desde
//   BreadcrumbList (§4, §8).
// - `price_*` desde Product.offers + `<title>` para el raw y la unit.
// - `product_name_original`, `product_description`, `image_primary`
//   desde Product JSON-LD (§4).
// - `supplier_city` desde regex `\bin\s+(...)\s*\|` sobre `<title>`
//   (§4, §7). El title real es `"... in {CITY} | ID: {PID}"`, no
//   `"in {CITY}, {STATE}."` como decía la spec vieja.
// - `supplier_state` = null con flag `supplier_state_from_home_needed`.
//   NO extraible del detail SSR — se resuelve en Stage 3 (§6).
// - `supplier_country` fijo `"IN"` (§4). v2 intentaba split(',')[2]
//   que devolvía undefined porque `.verT.fs13` solo trae city.
// - `supplier_id` tal-cual del slug del href del wrapper de `h2.fs15`
//   (vatsinternational sin guión). Hotfix de v2 ya lo traía — se
//   preserva.
// - `business_type`, `member_since_year` desde
//   `li.compdtsItems` (p.fs12.color1 label + h4.cmpfvalh4 value). v2
//   usaba `.company-details-grid dt/dd`, selector obsoleto que no
//   existe en el DOM actual.
// - `verified`, `trustseal` desde literal text en HTML raw. v2 usaba
//   `.slic.length > 0` (laxo) y `.slic .color1 span` (devolvía
//   "Mobile E-Mail").
// - MOQ con fallback embedded JSON primario: spec table HTML → regex
//   `PC_ITEM_MIN_ORDER_QUANTITY` sobre HTML raw. Flag
//   `moq_from_embedded_json` cuando se usa el fallback (§5, E5).
// - Spec table auxiliar (Form, Grade, NaOH Concentration, Purity,
//   CAS No, Packaging Type) → `appearance`, `grade`, `concentration`,
//   `cas_no`, `packaging_type` (§5).
// - `price_normalized_per_kg` según §5 (kg → price_min_usd; ton/mt →
//   /1000; resto → null).
// - `scraped_date` UTC ISO date (YYYY-MM-DD).
// - `scraper_flags` acumulado: `supplier_state_from_home_needed` (fijo),
//   `moq_from_embedded_json` (si corresponde),
//   `supplier_enrichment_pending` (fijo, Stage 3 pendiente),
//   `jsonld_parse_fallback`, `breadcrumb_missing`, `moq_missing`,
//   `spec_table_missing`, `currency_unexpected`, `price_unit_unknown`,
//   `supplier_location_missing`, `supplier_profile_missing`,
//   `name_clean_fallback` según §9.
// - NO emite entidad `supplier` — Stage 2 solo `product` con
//   supplier_* embedded como FK. Supplier entity completa = Stage 3
//   (§6, backlog §12).
//
// Reglas duras respetadas: sin try/catch salvo JSON.parse del loop
// JSON-LD (R7). `parse()` sin args (R9). `text_sane` (R11).
// `toArray().map` (R12). Wrappers `new URL` / `new Money` del runtime
// de BD Studio (ver made-in-china v3 como referencia del patrón).
//
// Shape del return: product row, §4+§5 completo (29 fields). El
// middleware aplica alias (`product_url` → `url`) en
// `_coerce_product` antes de emitir al envelope.
//
// NO tocar sc_browser (Stage 1 sano). NO tocar interaction_code_v2.js
// (guards alineados con §3 del handoff).
