// cosme v1 — parser code (sc_code / Stage 2 FETCH)
//
// Minimal surgical delta vs vendor:
//   (E7) breadcrumb Source 1 stored candidates as pre-wrapped `$(el)`
//        Cheerio instances in a plain Array, then called `.text()` on an
//        indexed element. BrightData Scraper Studio's preview rejected
//        the collector with:
//          "Can't save since preview errored on step 2:
//           Crawler error: (intermediate value).text is not a function"
//        Root cause: in BrightData's runtime, pushing a wrapped Cheerio
//        instance through `Array.prototype.map(...).filter(...)` and then
//        indexing it (`arr[arr.length-1]`) does NOT yield an object that
//        preserves `.text()` in all preview code paths. The fix is
//        conservative: keep the array as raw DOM elements and (re)wrap
//        with `$()` at the single point of use. This mirrors the R12
//        pattern already used everywhere else in the file.
//
//        Scope: only the 5 `.text()` calls at lines ~252 and ~259 (the
//        breadcrumb Source-1 block). No semantic change, no other
//        selectors touched.
//
// See `scrapers/cosme/sc_code/interaction_code_v1.js` header for context:
// bug E6 is scoped to Stage 1 discovery (sc_browser). Stage 2 product-level
// field extraction is otherwise unchanged and well-validated (fixtures
// 10248076, 10243030, 10259468 per vendor notes).
//
...
// The integration_code.js may pass shift_jis_fallback in input; accept it.
if (typeof input !== 'undefined' && input && input.shift_jis_fallback === true) {
...
if (has_mojibake && !scraper_flags.includes('shift_jis_fallback')) {
  // Integration didn't catch it but mojibake is visible → record it.
  // (We can't re-decode from inside parser; the integration owns that path.)
  scraper_flags.push('shift_jis_fallback');
}
